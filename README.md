...
Signature HTML de **398 × 301 px**, construite à partir de `Artwork/Signature_Mail_LEKO_2026_V7.ai` (plan de travail unique, source ×2 = 800 × 600, version finale du 17 septembre 2026, 00 h 08). Une seule mise en page pour tous les appareils : sa largeur tient dans la zone de lecture des téléphones sans réduction, le texte y garde sa taille réelle. Compatible Outlook (classique et nouveau, Windows et Mac), Gmail, Apple Mail, iOS/Android.
...
| `signature-m365.html` | Code compact pour une règle de flux Microsoft 365 (jetons d'annuaire, 2 206 caractères pour 5 000 autorisés). |
...
| `logo-white@4x.png` / `logo-black@4x.png` | 5 × 19 | ×4 (20 × 76) | Logo LEKO vertical (lecture de bas en haut) au bord droit de la tuile, sa base sur la ligne du téléphone ; blanc sur tuile noire, noir sur tuile blanche |
...
- **Texte** : prénom et nom sur une ligne en Medium 29 px en haut ; en bas, sur les mêmes lignes de base (Roman 9 px) : fonctions à gauche, e-mail souligné et téléphone alignés à droite en milieu de tuile, puis logo LEKO vertical 5 × 19 au bord droit. Interlettrage +0,2 / +0,28 px comme dans le fichier. Lignes de base vérifiées à ±0,2 px contre le plan de travail. Préréglage « Lisible » : fonctions et contacts 10 px.
...
Le fichier `.ai` est compatible PDF (PyMuPDF). Tuile noire `[10,10]–[785.8,295]` (rayon 15) : tranches `y 10–26` et `y 279–295` rendues à ×2 de la source avec fond transparent, variantes blanches par inversion RVB. Visuel : recadrage de la photo source correspondant à la fenêtre `[10,305]–[785.8,590]`, Lanczos vers 1164 × 429, masque arrondi 22,5 px, puis composition du CTA ombré (zone rendu après suppression de la photo. Logo : zone `[738.9,219.7]–[748.9,257.9]` du calque FIXED rendue à ×2 (20 × 76), variante noire par inversion RVB. CTA : zone `[685.2,489.6]–[783.2,587.6]`, posé en (1013, 277).
